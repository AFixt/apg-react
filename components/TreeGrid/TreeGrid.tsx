    ...
    switch (e.key) {
      ...
      case 'ArrowLeft':
        if (isFirstCol && row.hasChildren && expanded.has(row.id)) {
          ...
          const parentIdx = visible.findIndex((v) => v.id === row.parentId);
      ...
      case 'Home':
        ...
        break;
      case 'End':
        if (e.ctrlKey || e.metaKey) focusCell(visible.length - 1, totalCols - 1);
              ...
              return (
                ...
                >
                  {isFirst && row.hasChildren && (
                    <span aria-hidden="true" className="treegrid-chevron">
                      {isExpanded ? '\u25BE' : '\u25B8'}
                    </span>
                  )}
