...
describe('Accessibility contracts (no external a11y libs)', () => {
  ...
  describe('AlertDialog', () => {
    test('role=alertdialog, aria-modal=true, and proper labelling/description', () => {
      render(<AlertDialog isOpen title="Confirm" message="Are you sure?" onClose={() => {}} />);
  ...
  describe('Article', () => {
    test('native article landmark with heading as accessible name source', () => {
      ...
      assertAriaReferencesResolve(article, ['aria-labelledby']);
  ...
  describe('Breadcrumb', () => {
    test('nav landmark with aria-label, links with names, current page non-link', () => {
      ...
      screen.getAllByRole('link').forEach((l) => assertHasAccessibleName(l, 'breadcrumb link'));
  ...
  describe('Button', () => {
    ...
    test('toggle button surfaces valid aria-pressed', () => {
      render(<Button action={() => {}} label="Bold" isToggleButton toggleState />);
      const btn = screen.getByRole('button');
  ...
  describe('Checkbox', () => {
    test('has accessible name and associates label with input', () => {
      ...
      const labelEl = screen.getByText('Remember me').closest('label');
      expect(assertLabelAssociated(labelEl)).toBe(cb);
    });
    test('tri-state exposes aria-checked=mixed when indeterminate', () => {
      render(<Checkbox label="All" checked={null} isTriState onChange={() => {}} />);
  ...
  describe('MenuButton', () => {
    test('trigger has aria-haspopup, aria-expanded, aria-controls resolves when open', () => {
      render(<MenuButton label="Actions" items={[{ id: '1', label: 'One' }]} />);
      const btn = screen.getByRole('button');
      expect(btn).toHaveAttribute('aria-haspopup', 'menu');
      assertAriaBooleanState(btn, 'aria-expanded');
      fireEvent.click(btn);
      assertAriaReferencesResolve(btn, ['aria-controls']);
      screen.getAllByRole('menuitem').forEach((item) => assertHasAccessibleName(item));
    });
  });
  ...
  describe('Menubar', () => {
    test('menubar is labelled and top-level items have aria-haspopup=menu', () => {
      render(
        <Menubar
          label="Main"
          menus={[{ id: 'file', label: 'File', items: [{ id: 'new', label: 'New' }] }]}
        />,
      );
  ...
  describe('Spinbutton', () => {
    test('role=spinbutton with valuenow/min/max and keyboard focusable', () => {
      ...
      screen
        .getAllByRole('button')
        .forEach((btn) => assertHasAccessibleName(btn, 'spinner control'));
    });
  });
  ...
  describe('Textbox', () => {
    ...
    test('invalid textbox references its error via aria-errormessage', () => {
      render(
        <Textbox label="Email" value="bad" onChange={() => {}} invalid errorMessage="Invalid" />,
      );
      const input = screen.getByRole('textbox');
