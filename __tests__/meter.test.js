...
describe('Meter Component (APG meter pattern)', () => {
  ...
  test('uses userFriendlyText for aria-valuetext when provided', () => {
    ...
    render(<Meter value={65} label="Disk" userFriendlyText={friendly} />);
    expect(screen.getByRole('meter')).toHaveAttribute('aria-valuetext', '65 percent full');
  });
  ...
  test('renders a visual fill bar whose width reflects the value', () => {
    const { container } = render(<Meter value={40} minValue={0} maxValue={100} label="X" />);
