...
export function assertRole(el, expected) {
    ...
    if (role !== expected) {
        throw new Error(
            `Expected role="${expected}" but got role="${role}" on ${el.tagName}`
        );
    }
}
...
export function assertAriaBooleanState(el, attr) {
    ...
    if (val !== "true" && val !== "false") {
        throw new Error(
            `Expected ${attr} to be "true" or "false" on ${el.tagName}, got "${val}"`
        );
    }
}
...
export function assertRovingTabindex(elements) {
    ...
    const untabbable = elements.filter(
    ...
    if (tabbable.length + untabbable.length !== elements.length) {
        throw new Error(
            `All elements in roving group should have tabindex="0" or "-1"`
        );
    }
}
...
export function assertAriaReferencesResolve(el, attrs) {
    ...
    if (missing.length) {
        throw new Error(
            `Dangling ARIA references on ${el.tagName}: ${missing.join(", ")}`
        );
    }
}
