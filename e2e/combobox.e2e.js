...
describe('Combobox (E2E)', () => {
  test('typing filters the popup and Enter selects an option', async () => {
    const { page, close } = await openStory('components-combobox--autocomplete-list');
  ...
  test('Escape closes the popup', async () => {
    ...
    await page.waitForFunction(() => !document.querySelector("#storybook-root [role='listbox']"), {
      timeout: 5000,
    });
    await close();
  });
  ...
  test('aria-activedescendant points to a real option', async () => {
    ...
    const { activeId, optionExists } = await page.evaluate(() => {
      const input = document.querySelector("#storybook-root [role='combobox']");
      const activeId = input.getAttribute('aria-activedescendant');
      return {
        activeId,
        optionExists: activeId ? !!document.getElementById(activeId) : false,
      };
    });
