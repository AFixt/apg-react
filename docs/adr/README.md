...
Each ADR captures a single decision, its context, and its consequences. Number
files sequentially: `NNNN-short-slug.md`.

## Process

1. Copy `0000-template.md` to `NNNN-your-decision.md`.
2. Fill it in. Keep it short — one screen is often enough.
3. Open a PR. Reviewers can agree or push back on the decision before it is
   accepted.
4. Once merged, treat the ADR as immutable. To change a decision, write a new
   ADR that supersedes the previous one.
